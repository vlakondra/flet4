...
def onMenuClick(e):
    if e.control.content.value == "Открыть файл":
        ...
        files.current.on_result = onFileOpen
        files.current.pick_files(allow_multiple=False)

    elif e.control.content.value == "Сохранить данные":
        files.current.on_result = onSelectPath
        files.current.get_directory_path()
        # print(e.page.data["df"])


def onSelectPath(e):
    #Обработчик выбора папки для сохранения excel-файла.
    #Перед выполнением проверить , что в атрибуте data есть  df
    #и что пользователь выбрал папку.
    #Имя файла должно совпадать с именем таблицы. Реализуйте это!
    e.page.data['df'].to_excel(e.path+'\\'+'table.xlsx',engine='xlsxwriter')


# Обработик выбора файла
def onFileOpen(e):
    # e.page.data=e.files[0].path #сохраним путь к БД для дальнейшего использования
    e.page.data["path"] = e.files[0].path
    ...
    return ft.Container(
        ft.Row(
            controls=[
                ft.Container(
                    ft.Text(
                        str(row[col]), overflow=ft.TextOverflow.VISIBLE, no_wrap=False
                    ),
                    width=100,
                    padding=5,
                    expand=True,
...
def onDrawerClick(e, tbl):
    ...
    sqlite = SQLiteDB(e.page.data["path"])

    tname = tbl[e.control.selected_index]
    # получим DF выбранной таблицы
    tdf = sqlite.get_table(tname)
    e.page.data["df"] = tdf  # сохраним DataFrame для использования при сохранении
    ...
    header = ft.Container(
        ft.Row(
            controls=[
                ft.Container(
                    ft.Text(
                        col, weight=ft.FontWeight.BOLD, color=ft.Colors.BLUE_GREY_700
                    ),
                    width=100,
                    padding=5,
                    expand=True,
    ...
    e.page.add(
        ft.Column(
            controls=[
                ft.Text(f"Таблица {tname}", size=20, weight=ft.FontWeight.BOLD),
                header,
                ft.Container(
                    ft.ListView(controls=rows, expand=True, spacing=0, padding=0),
                    height=400,
                    border=ft.border.all(1, ft.Colors.GREY_300),
                ),
            ],
...
def main(page: ft.Page):
    ...
    #атрибут для сохранения нужных данных
    page.data = {"path": "", "df": None}
    ...
    # Обработчик on_result устанавливается отдельно для
    # выбора файла БД и выбора папки сохранения excel-файла 
